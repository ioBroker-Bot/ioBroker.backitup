...
/** Token document as stored in the adapter state and returned by the OAuth proxy */
interface AccessTokens {
    access_token?: string;
    refresh_token?: string;
    /** lifetime in seconds, as delivered by the OAuth endpoint */
    expires_in?: number;
    /** ISO timestamp derived from `expires_in` when the tokens are stored */
...
class TokenRefresher {
    ...
    accessToken: AccessTokens | undefined;
            ...
            if (state?.val) {
                ...
                if (this.accessToken) {
                    this.accessToken.access_token_expires_on = new Date(
                        Date.now() + (this.accessToken.expires_in as number) * 1_000,
                    ).toISOString();
    ...
    async refreshTokens(retriesLeft = 5): Promise<void> {
        ...
        if (expiresIn <= 0) {
            try {
                ...
                this.accessToken = response.data;
                (this.accessToken as AccessTokens).refresh_token = this.refreshToken;

                if (this.accessToken) {
                    this.accessToken.access_token_expires_on = new Date(
                        Date.now() + (this.accessToken.expires_in as number) * 1000,
                    ).toISOString();
                ...
                if (retriesLeft > 0) {
                    ...
                    this.adapter.log.info(
                        `Retrying token refresh in ${retryDelay / 1000} seconds... (${retriesLeft} retries left)`,
                    );
    ...
    /**
     * Not referenced anywhere - main.js uses the static overload below. It was missing the `await`
     * on the axios call, so reading `.data` off the pending promise threw every time; added here
     * because the method cannot be typed otherwise, and it is unreachable either way.
     */
