...
export function command(options: GotifyOptions, log: ioBroker.Logger, callback?: BackItUpScriptCallback): void {
    setTimeout(() => {
        if (
            ...
            if (!errors.length) {
                ...
                let errorMessage = buildErrorMessage(options, options.gotify.systemLang);
