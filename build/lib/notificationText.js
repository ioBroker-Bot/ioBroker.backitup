...
exports.buildStorageList = buildStorageList;
exports.buildHistoryErrorLine = buildHistoryErrorLine;
...
function mask(text, secret) {
    try {
        const formatted = secret.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
 ...
 */
function buildErrorMessage(options, systemLang) {
    const errors = options.context.errors;
    let errorMessage = (0, tools_1._)('Your backup was not completely created. Please check the errors!!', systemLang);
    errorMessage += '\n';
    if (errors.iobroker) {
 ...
 */
function buildStorageList(options, systemLang, underlineHeading = false) {
    const heading = (0, tools_1._)('Storage location', systemLang);
    let storageOptions = underlineHeading ? `\n\n__${heading}:__\n` : `\n\n${heading}:\n`;
    let storageNum = 1;
    if (options.ftp?.enabled) {
        const m = `${storageNum++}. ${(0, tools_1._)('FTP', systemLang)} (%h%d)%k`;
        storageOptions += m
    ...
    if (options.cifs?.enabled) {
        const m = `${storageNum++}. ${(0, tools_1._)(`NAS (${options.cifs.mountType})`, systemLang)} (%h%d)%k`;
        storageOptions += m
 ...
 */
function buildHistoryErrorLine(errors, systemLang) {
    let errorMessage = (0, tools_1._)('Backup error on: ', systemLang);
