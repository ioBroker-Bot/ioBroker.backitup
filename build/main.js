...
const utils = __importStar(require("@iobroker/adapter-core")); // Get common adapter utils
 ...
 */
const backupTimeSchedules = [];
let taskRunning = false;
let dropBoxTokenRefresher = null;
const bashDir = (0, node_path_1.join)(utils.getAbsoluteDefaultDataDir(), adapterName).replace(/\\/g, '/');
/**
...
async function startBackup(config, cb) {
    ...
    taskRunning = true;
    try {
        (0, execute_1.default)(adapter, config, err => {
            taskRunning = false;
            cb?.(err);
        });
 ...
 */
function readRunResult(type, onSuccess, onFailure) {
            ...
            if (id === `${adapter.namespace}.oneClick.iobroker` || id === `${adapter.namespace}.oneClick.ccu`) {
                ...
                if ((sysCheck && sysCheck.ready && sysCheck.ready === true) || adapter.config.cifsEnabled === true) {
                    ...
                    void startBackup(config, err => {
                        ...
                        if (adapter.config.slaveInstance &&
                            type === 'iobroker' &&
                    ...
                    void adapter.setState(`oneClick.${type}`, false, true);
                    void adapter.setState('output.line', `[EXIT] ${tools._('A local backup is currently not possible. Please check your System!', systemLang)}`, true);
                }
            }
        }
    });
    ...
    adapter.on('message', async (obj) => {
        if (obj) {
            switch (obj.command) {
                ...
                case 'authDropbox':
                    if (obj.callback) {
                        void tokenRefresher_1.default.getAuthUrl('https://oauth2.iobroker.in/dropbox').then(url => adapter.sendTo(obj.from, obj.command, { url }, obj.callback));
                    }
                    break;
                case 'authOnedrive': {
                    // eslint-disable-next-line @typescript-eslint/no-require-imports
                        ...
                        if (obj.message.type !== 'local') {
                            ...
                            _getFile.getFile(backupConfig, obj.message.type, obj.message.fileName, toSaveName, adapter.log, err => {
                                if (!err && (0, node_fs_1.existsSync)(toSaveName)) {
                                    try {
                                        adapter.sendTo(obj.from, obj.command, { fileName: fileName, listenPort: serverPort(dlServer) }, obj.callback);
                                    }
                                    catch (error) {
                    ...
                    if (obj?.message) {
                        if (adapter.config.hostType === 'Slave') {
                            ...
                            void startBackup(config, err => {
                                ...
                                timerOutput = setTimeout(() => readRunResult(type, value => {
                                    reply(value);
                ...
                case 'slaveInstance':
                    ...
                    break;
                case 'getLog': {
                    const logName = (0, node_path_1.join)(bashDir, `${adapter.namespace}.log`).replace(/\\/g, '/');
            ...
            backupTimeSchedules[type] = schedule.scheduleJob(cron, async () => {
                ...
                if ((sysCheck && sysCheck.ready && sysCheck.ready === true) || adapter.config.cifsEnabled === true) {
                    ...
                    void startBackup(backupConfig[type], err => {
                        ...
                        timerOutput2 = setTimeout(() => readRunResult(type, () => {
                            if (adapter.config.onedriveEnabled && adapter.config.hostType === 'Single') {
                                void renewOnedriveToken();
                            }
                        }), 500);
...
async function initConfig(secret) {
    ...
    if (adapter.config.dropboxEnabled) {
        dropBoxTokenRefresher = new tokenRefresher_1.default(adapter, 'info.dropboxTokens', 'https://oauth2.iobroker.in/dropbox');
        try {
    ...
    const storagesFor = (variant) => ({
        ftp: Object.assign({}, ftp, adapter.config.ftpOwnDir === true ? { dir: adapter.config[`ftp${variant}Dir`] } : {}),
        cifs: Object.assign({}, cifs, adapter.config.cifsOwnDir === true ? { dir: adapter.config[`cifs${variant}Dir`] } : {}),
        dropbox: Object.assign({}, dropbox, adapter.config.dropboxOwnDir === true ? { dir: adapter.config[`dropbox${variant}Dir`] } : {}),
        onedrive: Object.assign({}, onedrive, adapter.config.onedriveOwnDir === true ? { dir: adapter.config[`onedrive${variant}Dir`] } : {}),
        webdav: Object.assign({}, webdav, adapter.config.webdavOwnDir === true ? { dir: adapter.config[`webdav${variant}Dir`] } : {}),
        googledrive: Object.assign({}, googledrive, adapter.config.googledriveOwnDir === true ? { dir: adapter.config[`googledrive${variant}Dir`] } : {}),
    });
...
function createBashScripts() {
    ...
    const logFile = (0, node_path_1.join)(bashDir, `${adapter.namespace}.log`);
    ...
    if ((0, node_fs_1.existsSync)(`${bashDir}/.mount`)) {
        child_process.exec(`mount | grep -o "${backupDir}"`, (error, stdout) => {
            if (stdout.includes(backupDir)) {
                ...
                timerUmount1 = setTimeout(() => child_process.exec(`${adapter.config.sudoMount ? 'sudo umount' : 'umount'} ${backupDir}`, error => {
                    if (error) {
                        adapter.log.debug('umount: device is busy... wait 5 Minutes!!');
                        timerUmount2 = setTimeout(() => child_process.exec(`${adapter.config.sudoMount ? 'sudo umount' : 'umount'} -l ${backupDir}`, error => {
                            if (error) {
...
function setStartAll() {
    if (adapter.config.startAllRestore && !(0, node_fs_1.existsSync)(`${bashDir}/.startAll`)) {
        try {
    ...
    else if (!adapter.config.startAllRestore && (0, node_fs_1.existsSync)(`${bashDir}/.startAll`)) {
        try {
...
async function detectLatestBackupFile(adapter) {
    ...
    try {
        let stores = Object.keys(backupConfig.iobroker).filter(attr => typeof backupConfig.iobroker[attr] === 'object' &&
        ...
        const list = require('./lib/list');
        try {
            promises = stores.map(storage => new Promise(resolve => list(storage, backupConfig, adapter.log, result => {
...
async function renewOnedriveToken() {
    ...
    if (diffDays >= 30 || !adapter.config.onedriveLastTokenRenew) {
        ...
        void onedrive
            ...
            void adapter.registerNotification('backitup', 'onedriveWarn', err
                ? JSON.stringify(err)
                : 'An update of the Onedrive refresh token has failed. Please check your system!');
        });
    }
